...
ctypedef sym.Struct* StructPtr;
ctypedef sym.Union* UnionPtr;
ctypedef sym.Typedef* TypedefPtr;

cdef ConvBaseType(sym.BaseType* ptr):
	if not ptr:
		return
	cdef StructPtr ps = dynamic_cast[StructPtr](ptr)
	if ps:
		return Struct(<uintptr_t> ps)
	cdef UnionPtr pu = dynamic_cast[UnionPtr](ptr)
	if pu:
		return Union(<uintptr_t> pu)
	cdef TypedefPtr pt = dynamic_cast[TypedefPtr](ptr)
	if pt:
		return Typedef(<uintptr_t> pt)
...
cdef class SymbolManager:
	...
	def findFunctionByName(self, string name):
		"""Returns a L{Function} with the given name or None"""
		cdef sym.Function* ptr = self.sm_ptr.findFunctionByName(name)
		return Function(<uintptr_t> ptr) if ptr else None
	def findVariableByName(self, string name):
		"""Returns a L{Variable} with the given name or None"""
		cdef sym.Variable* ptr = self.sm_ptr.findVariableByName(name)
		return Variable(<uintptr_t> ptr) if ptr else None
	...
	def findBaseTypeByID(self, uint64_t typeID):
		"""Returns a type (Struct, Union, Typedef) with the given name or None"""
...
cdef class Symbol:
	...
	def __cinit__(self, uintptr_t ptr = 0):
		self.xownership = False
		if ptr != 0:
			self.Symbol_ptr = <sym.Symbol*> ptr
...
cdef class ReferencingType:
	...
	def __cinit__(self, uintptr_t ptr = 0):
		self.xownership = False
		if ptr != 0:
			self.ReferencingType_ptr = <sym.ReferencingType*> ptr
...
cdef class BaseType(Symbol):
	...
	def __cinit__(self, uintptr_t ptr = 0):
		self.xownership = False
		if ptr != 0:
			self.BaseType_ptr = <sym.BaseType*> ptr
...
cdef class RefBaseType(Symbol):
	...
	def __cinit__(self, uintptr_t ptr = 0):
		if ptr != 0:
			self.RefBaseType_ptr = <sym.RefBaseType*> ptr
...
cdef class ConstType(RefBaseType):
	...
	def __cinit__(self, uintptr_t ptr = 0):
		if ptr != 0:
			self.ConstType_ptr = <sym.ConstType*> ptr
...
cdef class Typedef(RefBaseType):
	...
	def __cinit__(self, uintptr_t ptr = 0):
		if ptr != 0:
			self.Typedef_ptr = <sym.Typedef*> ptr
...
cdef class FuncPointer(RefBaseType):
	...
	def __cinit__(self, uintptr_t ptr = 0):
		if ptr != 0:
			self.FuncPointer_ptr = <sym.FuncPointer*> ptr
...
cdef class Function(BaseType):
	...
	def __cinit__(self, uintptr_t ptr = 0):
		if ptr != 0:
			self.Function_ptr = <sym.Function*> ptr
...
#cdef class StructuredMember(Symbol, ReferencingType):
	...
	def __cinit__(self, uintptr_t ptr = 0):
		if ptr != 0:
			self.StructuredMember_ptr = <sym.StructuredMember*> ptr
	...
	def getBaseType(self):
		cdef sym.BaseType* ptr = self.StructuredMember_ptr.getBaseType()
		return ConvBaseType(ptr)
...
cdef class Structured(BaseType):
	...
	def __cinit__(self, uintptr_t ptr = 0):
		if ptr != 0:
			self.Structured_ptr = <sym.Structured*> ptr
...
cdef class Struct(Structured):
	...
	def __cinit__(self, uintptr_t ptr = 0):
		if ptr != 0:
			self.Struct_ptr = <sym.Struct*> ptr
...
#cdef class Variable(Symbol, ReferencingType):
	...
	def __cinit__(self, uintptr_t ptr = 0):
		if ptr != 0:
			self.Variable_ptr = <sym.Variable*> ptr
